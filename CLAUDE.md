...
The escalation gate is not a single confidence check. It evaluates multiple independent signals on every turn. Some triggers are **hard** (any one of them, on its own, fires the gate immediately) and some are **soft** (one alone is just a hint; two or more soft signals together fire the gate). Listing them explicitly so the design intent is auditable.

**1. Low confidence score (soft).** The specialist generates a self-rated confidence between 0 and 1. If it falls below the calibrated threshold, escalate. This is the least reliable trigger on its own, because models can be confidently wrong, so it should combine with other signals rather than fire alone.

**2. Explicit user request for a human (hard).** If the customer asks for a person, a manager, or an agent, escalate immediately. No friction, no "let me try one more thing." Non-negotiable for enterprise support.

**3. Sensitivity detection (hard).** Fires independently of confidence. Even if the agent is highly confident, certain categories always escalate: legal threats or legal questions, security incidents, churn risk (a high-value customer threatening to leave), compliance or regulatory topics, and financial disputes above a configurable threshold. Being wrong on these categories is too expensive, so confidence is irrelevant.

**4. Repeated failed resolution (hard).** If the agent has gone back and forth with the customer two or three times on the same issue without resolving it, it is stuck. Hard iteration limit, then escalate with the full context of what was already tried so the human agent does not start over.

**5. Verification pass rejection (hard).** The independent verifier (ADR-002) rejected a proposed state-changing action because it violates policy or does not match what the customer actually asked. If the specialist proposed something the verifier will not approve, escalate rather than retrying the same logic, because the specialist's judgment on this case is suspect.

**6. Knowledge gap or conflict (hard).** Retrieval returned nothing relevant, or it returned conflicting documents that the specialist cannot reconcile using recency and source authority. Rather than hallucinating a policy, escalate and flag the gap so the knowledge base can be updated.

**7. Critical tool failure (hard).** A tool required for resolution is unreachable or erroring. If the account-lookup API is down and the customer needs account-specific help, the agent cannot resolve the case. Escalate with a note that the issue is a tool outage, not a knowledge gap.

**8. Cost budget exceeded (hard).** The per-request token budget has been hit. Rather than continuing to burn tokens on a pathological request, escalate. This is a safety valve, not a common trigger.

**9. Frustrated customer detected (soft).** Sentiment analysis detects escalating frustration even without an explicit request for a human. This is a softer signal and should combine with other factors (failed resolution attempts, low confidence) rather than firing alone, because sentiment detection has false positives.

**Design principle: independent and additive.** These triggers are evaluated on every turn. Hard triggers (2, 3, 4, 5, 6, 7, 8) fire immediately on their own. Soft triggers (1 and 9) combine and fire only when multiple soft signals are present simultaneously. Every escalation produces a structured handoff payload containing the full conversation summary, which trigger fired, what was already tried, and any relevant retrieved context, so the human agent has the full picture and the customer does not have to repeat themselves.
...
- Tool call times out or errors: defined retry policy with backoff, then graceful fallback or escalation. Never a raw error to the customer. (See Section 4.4, trigger 7, for critical tool failures.)
...
- Conflicting knowledge base entries: retrieval surfaces the conflict rather than silently picking one; the specialist either reconciles using recency and source authority or escalates. (See Section 4.4, trigger 6.)
- Low-confidence loops (the agent keeps trying and keeps being unsure): hard iteration limit, then escalate. (See Section 4.4, trigger 4.)
...
- Cost runaway on a single pathological request: per-request token budget enforced; exceeding it triggers escalation. (See Section 4.4, trigger 8.)
- Repeat contact about the same unresolved issue: detected and prioritized for escalation rather than restarting the same failed loop. (See Section 4.4, trigger 4.)
- Customer explicitly frustrated or requesting a human: immediate escalation path, no friction. (See Section 4.4, triggers 2 and 9.)
...
**Decision.** The escalation gate evaluates nine independent triggers, not just confidence scoring. See Section 4.4 for the full list and the hard/soft split.
...
- (B) is brittle. Keyword lists miss novel phrasings, new edge cases require manual rule updates, and adversarial users find the gaps. Rules work as a hard backstop for specific categories (the sensitivity triggers in Section 4.4 use rules), but cannot be the whole gate.
